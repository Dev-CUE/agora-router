...
test('T5.6 — resolved가 라운드·발화보다 먼저 체크됨 (round=11, speaker=10 동시 도달)', () => {
  // round=11 → ROUND_LIMIT, speaker=10 → 다음 호출시 SPEAKER_LIMIT 둘 다 해당
  // resolved가 있으면 A2AResolved여야 함 (A2AError가 아님)
  assert.throws(
    () => validateA2A(
...
test('T5.8 — allowed_targets 위반 → A2A_UNAUTHORIZED', () => {
  ...
  assert.throws(
    () => validateA2A(
      ...
      { to: ['zeus'], cc: [] }, // zeus는 hermes의 허용 목록에 없음
      makePayload(),
      null
    ),
...
test('T5.11 — cc 에이전트 A2A 개시 → 차단 (can_initiate:false)', () => {
  // athena는 cc 역할 에이전트 (can_initiate: false)
  assert.throws(
    () => validateA2A(
      makeA2A({ caller: 'athena' }),
      { to: ['hera'], cc: [] },
...
test('T5.12 — 위조 caller → A2A_SPOOF_DETECTED', () => {
  // zeus URL은 'http://zeus-agent:3001', 위조 URL로 호출 시 차단
  assert.throws(
    () => validateA2A(
      makeA2A({ caller: 'zeus' }),
      { to: ['hera'], cc: [] },
...
test('T5.16 — 모드 미지정 → 기본값 single 적용', () => {
  const a2a = {
    ...
    max_speaker_calls: 10,
    max_rounds: 10,
    round: 11, // dialogue였다면 ROUND_LIMIT_EXCEEDED
    speaker_counts: {}
  };
  // 모드가 없으므로 single 취급 → 라운드 한도 미적용 → 성공해야 함
